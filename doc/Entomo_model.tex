...
\noindent\textit{Source: \href{https://mc-stan.org/learn-stan/case-studies/icar_stan.html}{Exact sparse CAR models in Stan} (Stan case study).}
...
For $B$ spatial units, the relationships are encoded in a $B \times B$ binary adjacency matrix $W$, where $W[b,b'] = 1$ if blocks $b$ and $b'$ share a boundary and 0 otherwise. The matrix is symmetric and has zeros on the diagonal. CAR has a parameter $\rho \in [0,1]$ controlling how strongly each block is pulled towards its neighbours:
$$ u_b \mid u_{-b} \sim \text{Normal}(\rho \cdot \bar{u}_b,\; \sigma^2/n_b), $$
When $\rho = 0$: no spatial correlation, blocks are independent.\\
When $\rho = 1$: maximum spatial smoothing --- this is the \textbf{intrinsic CAR (ICAR)}, which has one fewer free parameter (no $\rho$ to estimate) and enforces strong spatial smoothing.

\subsubsection{The conditional distribution}
For block b, the CAR model says: 
$$ u_b | u_{-b} \sim Normal(\mu_b, \tau_b^2), $$
where: 
$$ \bar{u}_b = \frac {\sum_{j \in \mathcal{N}(b)} w_{bj} \cdot u_j}{\sum_{j \in \mathcal{N}(b)} w_{bj}}  $$

$$ \tau_b^2 = \frac {\sigma^2}{\sum_{j \in \mathcal{N}(b)} w_{bj}}  $$

\noindent $\bar{u}_b$ is the \textbf{weighted average of b's neighbours' values}. $w_{bj}$ is the weight between block b and neighbour j - usually just 1 for all neighbours (binary adjacency). 

\noindent $\tau_b^2$ is the \textbf{conditional variance} - it strinks as block b has more neighbours (intuitively: if a block has more neighbours, you have more information about what its value should be). 
...
For convenience you can work with the precision matrix $Q = \Sigma^{-1}$ (the inverse of the covariance matrix) rather than the covariance itself. The joint distribution then becomes: 

$$ u \sim MVN(0,\cdot Q^{-1}) $$

\noindent Where $Q$ has a specific structure: 
$ Q[b,b] = n_b / \sigma^2$ (diagonal: number of neighbours of block b)
$ Q[b,b'] = -w_{bb'} / \sigma^2$ (off-diagonal: 1 for nieghbours, 0 otherwise)

In plain, the diagonal entries count how many neighbours each block has. The off-diagonal entries are -1 for neighouring pairs and 0 for non-neighbouring pairs. This is why CAR is computationally efficient. 

\subsubsection{introduction $\rho$ spatial autcorrelation parameter}
In the basic CAR model, the $\rho$ parameter controls \textbf{how strongly} the blocks are pulled towards their neighbouring values. 
$$ u_b | u_{-b} ~ Normal(\rho \cdot \bar{u}_b, \sigma^2/n_b), $$
where $\bar{u}_b$ is the simple average of the neighbours (assuming binary weights), which is what we will do in the model. 

The precision matrix becomes: 
$$Q = (D - \rho \cdot W)/ \sigma^2$$ 

Where: 
\begin{itemize}
  \item D is the diagnoal matrix $D[b,b] = n_b$ (number of neighbours)
  \item W is the adjacency matrix ($W[b,b']$ = 1 if neighbours, 0 otherwise)
  \item $\rho$ scales how much the off-diagonal (neighbour) structure matters
\end{itemize}

\textbf{What $\rho$ controls:} 
When $\rho$ = 0: $Q = D/\sigma^2$, D is diagonal, so Q is diagonal, meaning all blocks are independent. Each $u_b \sim Normal(0, \sigma^2/n_b)$.   
When $\rho \rightarrow 1$: $Q = (D-W)/\sigma^2$, this is maximum spatial smoothing, this limiting case is ICAR.  
When $\rho$ is between 0 and 1: This is between independence and spatial smoothing. $\rho$ can be estated from the data.    



\subsubsection{The log probability under CAR}
The joint distribution of \(u=(u_1,\ldots,u_B)\) is multivariate normal: 
$$ u \sim MVN(0, \sigma^2 \cdot(D-\rho \cdot W)^{-1}), $$  
\noindent The precision matrix is: 

$$Q = (D- \rho \cdot W)/\sigma^2$$

The log-probabiliy of an MVN is: 

$$\log(p(u)) = -B/2 \cdot \log(2\pi) + 1/2 \log|Q| - 1/2 \cdot u^T \cdot Q \cdot u,$$

\noindent Where $-B/2 \cdot \log(2\pi)$ is a normalizing constant, $1/2 \log|Q| = 1/2 \log(|(D - \rho \cdot W)\sigma^2|) =  1/2 \log(|D- \rho \cdot W|) - B\cdot \log(\sigma^2)$. The $\log(|D- \rho \cdot W|)$ is the hard part. This is the log-determinant of a \(B\times B\) sparse matrix. Computing it naively requires factorising $(D- \rho \cdot W)$ -- $O(B^3)$ operations for a dense matrix and taking the log of the product of diagonal elements. For B = 1336 this is $\pm$ 2.4 billion operations per HMC step.  
\paragraph{Why eigenvalues must be precomputed carefully.}
A common shortcut is to say ``precompute eigenvalues in R, then the log-determinant is $O(B)$ per HMC step.'' This is true but requires care, because the eigenvalues of $(D - \rho W)$ depend on $\rho$, which is sampled at every HMC step --- they cannot be precomputed directly.

What \emph{can} be precomputed are the eigenvalues of the $\rho$-free matrix $D^{-1/2} W D^{-1/2}$. Denote these $e_1, \ldots, e_B$. The eigenvalues of $(D - \rho W)$ are then:
\begin{equation}
  \lambda_m(\rho) = d_m \cdot (1 - \rho \cdot e_m),
\end{equation}
so the log-determinant decomposes as:
\begin{equation}
  \log|D - \rho W| = \underbrace{\sum_{m=1}^B \log(d_m)}_{\text{constant, precomputed}} + \underbrace{\sum_{m=1}^B \log(1 - \rho \cdot e_m)}_{O(B) \text{ per HMC step}}.
\end{equation}
The second sum depends on $\rho$ but only through a simple $O(B)$ loop over precomputed scalars $e_m$.
...
Operation & Cost & When \\
\midrule
Eigendecomposition of $D^{-1/2}WD^{-1/2}$ & $O(B^3)$ & Once in R --- never in Stan \\
