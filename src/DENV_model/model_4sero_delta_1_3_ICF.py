...
# new model + lagged sf 
###############################################################
class JumpProcess_SEIR4_beta_by_Temp_sf_BirthDeath_reported_d_1_3_ICF_lagged(JumpProcess):
    """
    Stochastic SEIR4 model for DENV with 4 serotypes
    No inclusion of age. All births go into susceptible compartment. All deaths leave from every possible compartment (they are natural deaths, unrelated to DENV). A fraction of all infected is considered "reported". 
    ...
    -- Parameters -- 
    alpha : temporary cross-immunity
    beta0 : transmission rate - baseline (before rescaling for seasonality)
    b : birth rate - constant for now
    d : death rate - constant for now and not the same as b
    sigma : incubation period
    gamma : infectious period
    ...
    rho:  proportion; reported cases / all infected (for secondary infections) (based on Imai et al. (2016))
    delta:  proportion; probability of detecting a primary/tertiary infection relative to a secondary infection (based on Imai et al. (2016))
    f_imm_1, f_imm_2, f_imm_3, f_imm_4 : fractions of the population that are assumed to be immune to serotype 1, 2, 3, 4 at the start of the simulation
    ---------------
    ...
    parameters = ['alpha','b', 'd', 'beta_0', 'sigma', 'gamma', 'psi', 'rho', 'delta_1', 'delta_3','lag', 'sf', 'epi_uf', 'f_imm_1', 'f_imm_2', 'f_imm_3', 'f_imm_4', 'pop_uf', 'compartment_fractions', 'states_list']
    ...
    def compute_rates(t, 
                      ...
                      b, d, beta_0, alpha, sigma, gamma, psi,rho, delta_1,delta_3,lag, sf, 
                      epi_uf, f_imm_1, f_imm_2, f_imm_3, f_imm_4, pop_uf, compartment_fractions, states_list):
        ...
        # set beta_t based on the time-dependent beta function
        beta_t = beta_0
        # print(f"[DEBUG] beta_t at t={t}: {beta_t}")
    ...
    def apply_transitionings(t, tau, transitionings, 
                            ...
                            b, d, beta_0, alpha, sigma, gamma, psi, rho, delta_1, delta_3,lag, sf, epi_uf, f_imm_1, f_imm_2, f_imm_3, f_imm_4, pop_uf, compartment_fractions, states_list):
...
################################
# TEMPORAL MODEL + 4 SEROTYPES AND BETA PER SEROTYPE
################################

class JumpProcess_SEIR4_beta1234_by_Temp_sf_BirthDeath_reported_d_1_3_ICF_lagged(JumpProcess):
    """
    Stochastic SEIR4 model for DENV with 4 serotypes
    No inclusion of age. All births go into susceptible compartment. All deaths leave from every possible compartment (they are natural deaths, unrelated to DENV). A fraction of all infected is considered "reported". 
    ...
    b : birth rate - constant for now
    d : death rate - constant for now and not the same as b
    beta_1, beta_2, beta_3, beta_4 : baseline transmission rates for each serotype before scaling for seasonality
    sigma : incubation period
    gamma : infectious period
    psi : enhanced / inhibited infectiousness of secondary infection
    rho:  proportion; reported cases / all infected (for secondary infections) (based on Imai et al. (2016))
    delta_1:  proportion; probability of detecting a primary infection relative to a secondary infection (based on Imai et al. (2016))
    delta_3:  proportion; probability of detecting a tertiary infection relative to a secondary infection (based on Imai et al. (2016))
    lag : time lag in time_unit between meteo data and its effect on dengue transmission (beta) -> not fully correct yet (uses weeks in load_epi_and_scaling and uses time_unit in time_dependent_beta)
    ---------------
    ...
    parameters = ['alpha','b', 'd', 'beta_1', 'beta_2', 'beta_3', 'beta_4', 'sigma', 'gamma', 'psi', 'rho', 'delta_1', 'delta_3','lag', 'sf', 'epi_uf', 'f_imm_1', 'f_imm_2', 'f_imm_3', 'f_imm_4', 'pop_uf', 'compartment_fractions', 'states_list']
    ...
    def compute_rates(t, 
                      ...
                      b, d, beta_1, beta_2, beta_3, beta_4, alpha, sigma, gamma, psi,rho, delta_1,delta_3,lag, sf, 
                      epi_uf, f_imm_1, f_imm_2, f_imm_3, f_imm_4, pop_uf, compartment_fractions, states_list):
        ...
        for sero in serotypes:
            ...
            sum_higher = np.sum(state_array[higher_idxs]) if higher_idxs else 0
            # calculate the force of infection for this serotype
            beta_sero = beta_t[sero]
            if np.isnan(T) or T == 0:
                print(f"[DEBUG] T is invalid at t={t}: T={T}, state_array={state_array[:5]}, beta_sero={beta_sero}")
            else:
                # forces[sero] = beta_sero * (primary_val + psi * sum_higher) / T
                # clip forces to be between 0 and 1 -> to avoid ValueError: p < 0, p > 1 or p contains NaNs
                # REPLACED THE FOLLOWING TO COUNT NUMBER OF CLIPPED VALUES
                forces[sero] = np.clip(beta_sero * (primary_val + psi * sum_higher) / T, 0, 1)
                # # WITH THIS 
                # unclipped = beta_sero * (primary_val + psi * sum_higher) / T
                # clipped = np.clip(unclipped, 0, 1)
                # n_clipped = np.sum((unclipped < 0) | (unclipped > 1))
                # if n_clipped > 0:
                #     print(f"[CLIP] {n_clipped} value(s) clipped for serotype {sero} at t={t}")
                # forces[sero] = clipped
                # # TILL HERE CAN BE REMOVED LATER
        ...
        # suscptibles
            if state.startswith("S"):
                ...
                rates[state] = [
                    np.clip((1 / gamma) * np.ones(state_array[idx].shape), 0,1), # avoid rates >1 for infectious period
                    d * np.ones(state_array[idx].shape)]
                # # CAN BE REMOVED ONCE CLIPPING HAS BEEN COUNTED
                # unclipped = (1 / gamma) * np.ones(state_array[idx].shape)
                # clipped = np.clip(unclipped, 0, 1)
                # n_clipped = np.sum((unclipped < 0) | (unclipped > 1))
                # if n_clipped > 0:
                #     print(f"[CLIP] {n_clipped} value(s) clipped for state {state} at t={t}")
                # rates[state] = [
                #     clipped, # avoid rates >1 for infectious period
                #     d * np.ones(state_array[idx].shape)]
                # # CAN BE REMOVED ONCE CLIPPING HAS BEEN COUNTED
    ...
    def apply_transitionings(t, tau, transitionings, 
                            ...
                            b, d, beta_1, beta_2, beta_3, beta_4, alpha, sigma, gamma, psi, rho, delta_1, delta_3, lag, sf, epi_uf, f_imm_1, f_imm_2, f_imm_3, f_imm_4, pop_uf, compartment_fractions, states_list):
...
############################################################
# TEMPORAL MODEL + 1 BETA + FRACTION SEROT  1 - 4 OF INITIAL INFECTED AS PARAMETER
############################################################
class JumpProcess_SEIR4_beta_by_Temp_sf_BirthDeath_reported_d_1_3_ICF_imm_cases_lagged(JumpProcess):
    """
    Stochastic SEIR4 model for DENV with 4 serotypes
    No inclusion of age. All births go into susceptible compartment. All deaths leave from every possible compartment (they are natural deaths, unrelated to DENV). A fraction of all infected is considered "reported". 
    ...
    delta:  proportion; probability of detecting a primary/tertiary infection relative to a secondary infection (based on Imai et al. (2016))
    f_imm_1, f_imm_2, f_imm_3, f_imm_4 : fractions of the population that are assumed to be immune to serotype 1, 2, 3, 4 at the start of the simulation
    logit_f_c_1, logit_f_c_2, logit_f_c_3 : logit-transformed fractions of the initial infected that are assumed to be infected with serotype 1, 2, 3 at the start of the simulation. f_c_4 is computed via softmax to ensure sum=1.
    ---------------
    ...
    parameters = ['alpha','b', 'd', 'beta_0', 'sigma', 'gamma', 'psi', 'rho', 'delta_1', 'delta_3','lag', 'sf', 'epi_uf', 'f_imm_1', 'f_imm_2', 'f_imm_3', 'f_imm_4', 'logit_f_c_1', 'logit_f_c_2', 'logit_f_c_3', 'pop_uf', 'compartment_fractions', 'states_list']
    ...
    def compute_rates(t, 
                      ...
                      b, d, beta_0, alpha, sigma, gamma, psi,rho, delta_1,delta_3,lag, sf, 
                      epi_uf, f_imm_1, f_imm_2, f_imm_3, f_imm_4, logit_f_c_1, logit_f_c_2, logit_f_c_3, pop_uf, compartment_fractions, states_list):
    ...
    def apply_transitionings(t, tau, transitionings, 
                            ...
                            b, d, beta_0, alpha, sigma, gamma, psi, rho, delta_1, delta_3,lag, sf, epi_uf, f_imm_1, f_imm_2, f_imm_3, f_imm_4, logit_f_c_1, logit_f_c_2, logit_f_c_3, pop_uf, compartment_fractions, states_list):
