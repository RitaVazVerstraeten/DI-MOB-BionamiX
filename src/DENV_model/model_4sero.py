...
class JumpProcess_SEIR4_beta_by_Temp_sf_BirthDeath_reported(JumpProcess):
    ...
    parameters = ['alpha','b', 'd', 'beta_0', 'sigma', 'gamma', 'psi', 'rho', 'delta', 'sf'] 
    ...
    def compute_rates(t, 
                      ...
                      b, d, beta_0, alpha, sigma, gamma, psi,rho, delta, sf):
        ...
        # # After rates have been computed for all states
        ...
        #             print(f"State: {state}, Rate index: {i}, Values: {arr[arr < 0] if np.any(arr < 0) else ''} {arr[arr > 1] if np.any(arr > 1) else ''} {'NaN' if np.any(np.isnan(arr)) else ''}")
        ...
        # Derivative states
                ...
                for s in states_list:
                    if s.startswith("E"):
                        ...
                        if len(I_state) == 3:
                            ...
                            transitions += rho* delta * transitionings[s][0]
            ...
            # get the number of reported cases per serotype (I_1_rep, I_2_rep, etc.)
                ...
                for E_state in E_states:
                    ...
                    I_state = "I" + E_state[1:]
                    # Determine if this is a secondary infection (length 3, e.g. I21, I32)
                    if len(I_state) == 3: # for secondary infections
                        transitions += rho * transitionings[E_state][0]
                    else: # for all primary tertiary and quaternary infections
                        transitions += rho* delta * transitionings[E_state][0]
                new_state_array[idx] = transitions
