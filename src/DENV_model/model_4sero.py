# DENV model with 4 serotypes & ZIKV
...
class JumpProcess_SEIR4_beta_by_Temp_sf_BirthDeath_reported(JumpProcess):
    """
    ...
    rho:  percentage; reported cases / all infected (constant over time)
    ---------------
    ...
    def compute_rates(t, 
                      ...
                      b, d, beta_0, alpha, sigma, gamma, psi,rho, sf):
        ...
        serotypes = JumpProcess_SEIR4_beta_by_Temp_sf_BirthDeath_reported.serotypes
        ...
        primary_idx_by_sero = JumpProcess_SEIR4_beta_by_Temp_sf_BirthDeath_reported.primary_idx_by_sero
        ...
        exclude_states = {'I_new', 'I_cum', 'I_rep'}
        ...
        # suscptibles
            ...
            elif state.startswith("I") and state not in ["I_new", "I_cum", "I_rep"]:
                rates[state] = [
    ...
    def apply_transitionings(t, tau, transitionings, 
        ...
        # recovered                
                ...
                if len(state) > 1:
                    ...
                    new_state_array[idx] = (
                        ...
                        + sum(transitionings[s][0] for s in states_list if s.startswith("I") and len(s) > 4 and s not in ["I_new", "I_cum", "I_rep"]) # sum of all I states with 4 serotype exposure
        ...
        # Derivative states
                ...
                new_state_array[idx] = rho * new_state_array[state_name_to_idx["I_new"]]
...
JumpProcess_SEIR4_beta_by_Temp_sf_BirthDeath_reported.infectious_states_by_sero = {
    sero: [i for i, state in enumerate(JumpProcess_SEIR4_beta_by_Temp_sf_BirthDeath_reported.states_list)
           if state.startswith("I") and state.endswith(sero)]
    for sero in ["1", "2", "3", "4"]
}
JumpProcess_SEIR4_beta_by_Temp_sf_BirthDeath_reported.primary_idx_by_sero = {
    sero: JumpProcess_SEIR4_beta_by_Temp_sf_BirthDeath_reported.state_name_to_idx[f"I{sero}"]
    for sero in ["1", "2", "3", "4"]
}