...
# Variables
samples_path='optimization_SF/'
fig_path='optimization_SF/'
...
########## TURN COUNTS INTO A WEEKLY DATASET CUMMULATED ON SATURDAYS #######
counts_weekly = counts.resample('W-SAT').sum()
# reset start_date and end_date to match weekly dates: 
start_date = counts_weekly.index.min()
...
if __name__ == '__main__':    
    ...
    results, ax = variance_analysis(counts_weekly, resample_frequency='3W') # resample_frequency is window length 
    alpha = results.loc['negative binomial', 'theta']
    print(results)
    plt.show()
    plt.close()

# Negative binomial is most appropriate -> lowest AIC with argument = 0.885382 -> 0.440015  for weekly data

# je kan neg binom gebruiken met alpha = 0.05 -> conservatieve schatting. Als de pieken dan niet lukken om te fitten, gebruik Poisson. 
...
scaling_factors_weekly = {
    model: df['sf'].resample('W-SAT').mean()  # Resample from Saturday to Saturday and take the mean
...
# reset the end_date to match the weekly structure and show the minimal date between counts and sf
...
# make sure that the indexer sf_t = self.sf.loc[t, 'sf'] in time_dependent_parameters would work: 
params["sf"] = params["sf"].to_frame()

# zet initiele conditie gelijk aan 1/rho * I_reported en verdeel deze mensen over I1, I2, I12, en I21: 
initial_infected = (1/params["rho"])*counts_filtered[start_date]
...
####################################################################
# for scaling_factor = seasonal_forcing 
####################################################################
...
if __name__ == '__main__':
    ...
    # TEST USING INFORMATIVE PRIOR FOR ALPHA
    ...
    objective_function = log_posterior_probability(model,pars,bounds,data,states,
                                                    ...
                                                    simulation_kwargs={'tau':tau, 'time_unit': time_unit}, # 
                                                    labels=labels)
