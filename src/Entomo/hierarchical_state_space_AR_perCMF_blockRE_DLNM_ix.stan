// Per-CMF AR(1) + iid block random effect + DLNM cross-basis + interaction cross-bases.
// Extends hierarchical_state_space_AR_perCMF_blockRE_DLNM.stan with an optional
// interaction matrix X_ix (N x P_ix): binary modifier variables scaled into the
// DLNM cross-basis of a target predictor. Set P_ix = 0 to disable interactions.
//
// Build X_ix in R via build_dlnm_stan_data() with cfg$dlnm_ix_vars set. For each
// (binary_var, dlnm_var) pair the binary indicator is multiplied column-wise into
// the dlnm_var sub-block of X_cb and the results are cbind-ed into X_ix.
//
// AR(1) re-parameterised as (tau, rho): tau = marginal stationary SD of the AR(1)
// process; sigma_v = tau * sqrt(1 - rho^2) is derived. This avoids the 1/sqrt(1-rho^2)
// blow-up that caused low E-BFMI in the (sigma_v, rho) form.
data {
  int<lower=1> N;
